...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	(
		...
		g_sz_CLASSNAME,
		g_sz_CLASSNAME,
		WS_OVERLAPPEDWINDOW^WS_THICKFRAME^WS_MAXIMIZEBOX,//Окно является перекрывающимся. Перекрывающееся окно имеет заголовок окна и границу. //^ - убирает флаги
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		{
			//Буфер
			if (input) // усли ввод цифр
			{
				//берем заначение с экрана преобраз в число 
				CHAR sz_buffer[MAX_PATH]{};
				SendMessage(hEdit, WM_GETTEXT, MAX_PATH, (LPARAM)sz_buffer);
				//Переменная b
				b = atof(sz_buffer);//преобразование строки в double
				input = false;
				if (a == 0) a = b; // если левая операнда пустая
			}
			//input = false;
			if (input_operation)SendMessage(hwnd, WM_COMMAND, IDC_BUTTON_EQUAL, 0);
			operation = LOWORD(wParam); //сохраняем операцию
			input_operation = true; // операция была введена
		}
		if (LOWORD(wParam) == IDC_BUTTON_EQUAL)
		{
			...
			input_operation = false;//операция выполнена
			CHAR sz_buffer[MAX_PATH]{};
			sprintf(sz_buffer, "%g", a);//"%g" - округляет
			SendMessage(hEdit, WM_SETTEXT, 0, (LPARAM)sz_buffer);
		}
	...
	break;
	case WM_KEYDOWN://Кейс назыатий с клавиатуры
	{
	...
	case WM_CONTEXTMENU:
	{
		...
		InsertMenu(hMenu, 0, MF_BYPOSITION | MF_STRING, IDC_ROUND_GREEN, "Square green");
		switch (TrackPopupMenuEx(hMenu, TPM_BOTTOMALIGN | TPM_LEFTALIGN | TPM_RETURNCMD, LOWORD(lParam), HIWORD(), hwnd, NULL));
		{
		case IDC_SQUARE_BLUE: strcpy(sz_skin, "square_blue"); break;
		case IDC_SQUARE_GREEN: strcpy(sz_skin, "square_green"); break;
		case IDC_ROUND_GREEN: strcpy(sz_skin, "round_blue"); break; 
...
VOID SetSkin(HWND hwnd, CONST CHAR sz_skin[])
{
	...
	for (int i = 0; i < SIZE; i++)
	{
		...
		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%i.bmp", i);
	...
	for (int i = 0; i < sizeof(BUTTON_NAMES) / sizeof(BUTTON_NAMES[0]); i++)
	{
		...
		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%s.bmp", BUTTON_NAMES[i]);
		HBITMAP hBitmap = (HBITMAP)LoadImage	
		(
			GetModuleHandle(NULL),
			sz_filename,
